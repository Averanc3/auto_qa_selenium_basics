...
def test_user_registration(browser, base_url):
    ...
    with allure.step('Complete registration and make sure the new user actually exists'):
...
def test_switch_currency(browser, base_url):
    ...
    with allure.step('Make sure the exact products price is now present in different currency'):
